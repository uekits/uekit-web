...
├─ fs-utils.ts          # 文件系统工具
├─ file-transaction.ts  # 原子文件提交与失败回滚
└─ types.ts             # 公共内部类型
```
...
CLI 根据执行环境以及当前目录、工作区祖先目录中的锁文件识别 pnpm、npm、Yarn 或 Bun。依赖由 Registry 条目声明，CLI 负责去重并调用对应安装命令。当前正式消费矩阵验证 pnpm 和 npm；Yarn、Bun 属于已识别但尚未纳入正式矩阵的能力。

使用 `--skip-install` 可以只写源码和元数据，适用于离线环境或由消费方统一管理依赖的场景，但之后必须手工安装缺失依赖。
...
重复执行应尽量保持幂等，不应重复插入 Vite 插件或主题 import。安装计划内部的源码、Base 和 Lock 使用事务保护；Vite 配置和主题入口注入属于安装后的配置步骤，无法安全识别配置结构时会要求人工处理。
...
- 三方合并主要依赖 Git 文本合并。
- 尚未支持 `add item@version`、`install --frozen-lockfile` 和 Registry 级回滚。
- 同一消费项目不能并存同一 Registry 条目的多个版本。
