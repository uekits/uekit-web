# 第三方许可证清单

> 此文件由 `pnpm-lock.yaml`、`scripts/third-party-license-metadata.json` 和 `scripts/generate-third-party-licenses.mjs` 生成；不得手工编辑。

共复核 555 个 package/version，汇总为 499 条许可证记录。依赖仍受各自许可证原文约束。

| Package | Version | License |
| --- | --- | --- |
| @amap/amap-jsapi-loader | 1.0.1 | MIT |
| @babel/code-frame | 7.29.7 | MIT |
| @babel/helper-string-parser | 7.29.7 | MIT |
| @babel/helper-validator-identifier | 7.29.7 | MIT |
| @babel/parser | 7.29.7 | MIT |
| @babel/runtime | 7.29.7 | MIT |
| @babel/types | 7.29.7 | MIT |
| @cacheable/memory | 2.2.0 | MIT |
| @cacheable/utils | 2.5.0 | MIT |
| @changesets/apply-release-plan | 7.1.1 | MIT |
| @changesets/assemble-release-plan | 6.0.10 | MIT |
| @changesets/changelog-git | 0.2.1 | MIT |
| @changesets/cli | 2.31.0 | MIT |
| @changesets/config | 3.1.4 | MIT |
| @changesets/errors | 0.2.0 | MIT |
| @changesets/get-dependents-graph | 2.1.4 | MIT |
| @changesets/get-release-plan | 4.0.16 | MIT |
| @changesets/get-version-range-type | 0.4.0 | MIT |
| @changesets/git | 3.0.4 | MIT |
| @changesets/logger | 0.1.1 | MIT |
| @changesets/parse | 0.4.3 | MIT |
| @changesets/pre | 2.0.2 | MIT |
| @changesets/read | 0.6.7 | MIT |
| @changesets/should-skip-package | 0.1.2 | MIT |
| @changesets/types | 4.0.1, 6.1.0 | MIT |
| @changesets/write | 0.4.0 | MIT |
| @csstools/css-calc | 3.2.1 | MIT |
| @csstools/css-parser-algorithms | 4.0.0 | MIT |
| @csstools/css-syntax-patches-for-csstree | 1.1.6 | MIT-0 |
| @csstools/css-tokenizer | 4.0.0 | MIT |
| @csstools/media-query-list-parser | 5.0.0 | MIT |
| @csstools/selector-resolve-nested | 4.0.0 | MIT-0 |
| @csstools/selector-specificity | 6.0.0 | MIT-0 |
| @ctrl/tinycolor | 4.2.0 | MIT |
| @element-plus/icons-vue | 2.3.2 | MIT |
| @emnapi/core | 1.11.1 | MIT |
| @emnapi/runtime | 1.11.1 | MIT |
| @emnapi/wasi-threads | 1.2.2 | MIT |
| @eslint-community/eslint-utils | 4.9.1 | MIT |
| @eslint-community/regexpp | 4.12.2 | MIT |
| @eslint/config-array | 0.23.5 | Apache-2.0 |
| @eslint/config-helpers | 0.6.0 | Apache-2.0 |
| @eslint/core | 1.2.1 | Apache-2.0 |
| @eslint/js | 10.0.1 | MIT |
| @eslint/object-schema | 3.0.5 | Apache-2.0 |
| @eslint/plugin-kit | 0.7.2 | Apache-2.0 |
| @floating-ui/core | 1.7.5 | MIT |
| @floating-ui/dom | 1.7.6 | MIT |
| @floating-ui/utils | 0.2.11 | MIT |
| @humanfs/core | 0.19.2 | Apache-2.0 |
| @humanfs/node | 0.16.8 | Apache-2.0 |
| @humanfs/types | 0.15.0 | Apache-2.0 |
| @humanwhocodes/module-importer | 1.0.1 | Apache-2.0 |
| @humanwhocodes/retry | 0.4.3 | Apache-2.0 |
| @inquirer/external-editor | 1.0.2 | MIT |
| @isaacs/cliui | 8.0.2 | ISC |
| @jridgewell/gen-mapping | 0.3.13 | MIT |
| @jridgewell/remapping | 2.3.5 | MIT |
| @jridgewell/resolve-uri | 3.1.2 | MIT |
| @jridgewell/sourcemap-codec | 1.5.5 | MIT |
| @jridgewell/trace-mapping | 0.3.31 | MIT |
| @keyv/bigmap | 1.3.1 | MIT |
| @keyv/serialize | 1.1.1 | MIT |
| @lucide/vue | 1.24.0 | ISC |
| @manypkg/find-root | 1.1.0 | MIT |
| @manypkg/get-packages | 1.1.3 | MIT |
| @microsoft/tsdoc | 0.16.0 | MIT |
| @microsoft/tsdoc-config | 0.18.1 | MIT |
| @napi-rs/wasm-runtime | 1.1.6 | MIT |
| @nodelib/fs.scandir | 2.1.5 | MIT |
| @nodelib/fs.stat | 2.0.5 | MIT |
| @nodelib/fs.walk | 1.2.8 | MIT |
| @one-ini/wasm | 0.1.1 | MIT |
| @oxc-project/types | 0.139.0 | MIT |
| @parcel/watcher | 2.5.6 | MIT |
| @parcel/watcher-android-arm64 | 2.5.6 | MIT |
| @parcel/watcher-darwin-arm64 | 2.5.6 | MIT |
| @parcel/watcher-darwin-x64 | 2.5.6 | MIT |
| @parcel/watcher-freebsd-x64 | 2.5.6 | MIT |
| @parcel/watcher-linux-arm-glibc | 2.5.6 | MIT |
| @parcel/watcher-linux-arm-musl | 2.5.6 | MIT |
| @parcel/watcher-linux-arm64-glibc | 2.5.6 | MIT |
| @parcel/watcher-linux-arm64-musl | 2.5.6 | MIT |
| @parcel/watcher-linux-x64-glibc | 2.5.6 | MIT |
| @parcel/watcher-linux-x64-musl | 2.5.6 | MIT |
| @parcel/watcher-win32-arm64 | 2.5.6 | MIT |
| @parcel/watcher-win32-ia32 | 2.5.6 | MIT |
| @parcel/watcher-win32-x64 | 2.5.6 | MIT |
| @pkgjs/parseargs | 0.11.0 | MIT |
| @playwright/test | 1.61.1 | Apache-2.0 |
| @rolldown/binding-android-arm64 | 1.1.5 | MIT |
| @rolldown/binding-darwin-arm64 | 1.1.5 | MIT |
| @rolldown/binding-darwin-x64 | 1.1.5 | MIT |
| @rolldown/binding-freebsd-x64 | 1.1.5 | MIT |
| @rolldown/binding-linux-arm-gnueabihf | 1.1.5 | MIT |
| @rolldown/binding-linux-arm64-gnu | 1.1.5 | MIT |
| @rolldown/binding-linux-arm64-musl | 1.1.5 | MIT |
| @rolldown/binding-linux-ppc64-gnu | 1.1.5 | MIT |
| @rolldown/binding-linux-s390x-gnu | 1.1.5 | MIT |
| @rolldown/binding-linux-x64-gnu | 1.1.5 | MIT |
| @rolldown/binding-linux-x64-musl | 1.1.5 | MIT |
| @rolldown/binding-openharmony-arm64 | 1.1.5 | MIT |
| @rolldown/binding-wasm32-wasi | 1.1.5 | MIT |
| @rolldown/binding-win32-arm64-msvc | 1.1.5 | MIT |
| @rolldown/binding-win32-x64-msvc | 1.1.5 | MIT |
| @rolldown/pluginutils | 1.0.1 | MIT |
| @rollup/pluginutils | 5.4.0 | MIT |
| @sindresorhus/merge-streams | 4.0.0 | MIT |
| @standard-schema/spec | 1.1.0 | MIT |
| @sxzz/popperjs-es | 2.11.8 | MIT |
| @tailwindcss/node | 4.3.2 | MIT |
| @tailwindcss/oxide | 4.3.2 | MIT |
| @tailwindcss/oxide-android-arm64 | 4.3.2 | MIT |
| @tailwindcss/oxide-darwin-arm64 | 4.3.2 | MIT |
| @tailwindcss/oxide-darwin-x64 | 4.3.2 | MIT |
| @tailwindcss/oxide-freebsd-x64 | 4.3.2 | MIT |
| @tailwindcss/oxide-linux-arm-gnueabihf | 4.3.2 | MIT |
| @tailwindcss/oxide-linux-arm64-gnu | 4.3.2 | MIT |
| @tailwindcss/oxide-linux-arm64-musl | 4.3.2 | MIT |
| @tailwindcss/oxide-linux-x64-gnu | 4.3.2 | MIT |
| @tailwindcss/oxide-linux-x64-musl | 4.3.2 | MIT |
| @tailwindcss/oxide-wasm32-wasi | 4.3.2 | MIT |
| @tailwindcss/oxide-win32-arm64-msvc | 4.3.2 | MIT |
| @tailwindcss/oxide-win32-x64-msvc | 4.3.2 | MIT |
| @tailwindcss/vite | 4.3.2 | MIT |
| @tybys/wasm-util | 0.10.3 | MIT |
| @types/chai | 5.2.3 | MIT |
| @types/deep-eql | 4.0.2 | MIT |
| @types/esrecurse | 4.3.1 | MIT |
| @types/estree | 1.0.9 | MIT |
| @types/json-schema | 7.0.15 | MIT |
| @types/lodash | 4.17.24 | MIT |
| @types/lodash-es | 4.17.12 | MIT |
| @types/node | 12.7.1, 22.20.1 | MIT |
| @types/npm-package-arg | 6.1.4 | MIT |
| @types/semver | 7.7.1 | MIT |
| @types/web-bluetooth | 0.0.21 | MIT |
| @types/whatwg-mimetype | 3.0.2 | MIT |
| @types/ws | 8.18.1 | MIT |
| @typescript-eslint/eslint-plugin | 8.63.0 | MIT |
| @typescript-eslint/parser | 8.63.0 | MIT |
| @typescript-eslint/project-service | 8.56.1, 8.63.0 | MIT |
| @typescript-eslint/scope-manager | 8.56.1, 8.63.0 | MIT |
| @typescript-eslint/tsconfig-utils | 8.56.1, 8.63.0 | MIT |
| @typescript-eslint/type-utils | 8.63.0 | MIT |
| @typescript-eslint/types | 8.56.1, 8.63.0 | MIT |
| @typescript-eslint/typescript-estree | 8.56.1, 8.63.0 | MIT |
| @typescript-eslint/utils | 8.56.1, 8.63.0 | MIT |
| @typescript-eslint/visitor-keys | 8.56.1, 8.63.0 | MIT |
| @vitejs/plugin-vue | 6.0.7 | MIT |
| @vitest/expect | 4.1.10 | MIT |
| @vitest/mocker | 4.1.10 | MIT |
| @vitest/pretty-format | 4.1.10 | MIT |
| @vitest/runner | 4.1.10 | MIT |
| @vitest/snapshot | 4.1.10 | MIT |
| @vitest/spy | 4.1.10 | MIT |
| @vitest/utils | 4.1.10 | MIT |
| @volar/language-core | 2.4.28 | MIT |
| @volar/source-map | 2.4.28 | MIT |
| @volar/typescript | 2.4.28 | MIT |
| @vue/compiler-core | 3.5.39 | MIT |
| @vue/compiler-dom | 3.5.39 | MIT |
| @vue/compiler-sfc | 3.5.39 | MIT |
| @vue/compiler-ssr | 3.5.39 | MIT |
| @vue/language-core | 3.3.7 | MIT |
| @vue/reactivity | 3.5.39 | MIT |
| @vue/runtime-core | 3.5.39 | MIT |
| @vue/runtime-dom | 3.5.39 | MIT |
| @vue/server-renderer | 3.5.39 | MIT |
| @vue/shared | 3.5.39 | MIT |
| @vue/test-utils | 2.4.11 | MIT |
| @vueuse/core | 14.3.0 | MIT |
| @vueuse/metadata | 14.3.0 | MIT |
| @vueuse/shared | 14.3.0 | MIT |
| abbrev | 2.0.0 | ISC |
| acorn | 8.17.0 | MIT |
| acorn-jsx | 5.3.2 | MIT |
| ajv | 6.15.0, 8.18.0, 8.20.0 | MIT |
| alien-signals | 3.2.1 | MIT |
| ansi-colors | 4.1.3 | MIT |
| ansi-escapes | 7.3.0 | MIT |
| ansi-regex | 5.0.1, 6.2.2 | MIT |
| ansi-styles | 4.3.0, 6.2.3 | MIT |
| argparse | 1.0.10 | MIT |
| argparse | 2.0.1 | Python-2.0 |
| array-union | 2.1.0 | MIT |
| assertion-error | 2.0.1 | MIT |
| astral-regex | 2.0.0 | MIT |
| async-validator | 4.2.5 | MIT |
| balanced-match | 1.0.0, 4.0.4 | MIT |
| better-path-resolve | 1.0.0 | MIT |
| boolbase | 1.0.0 | ISC |
| brace-expansion | 2.1.2, 5.0.7 | MIT |
| braces | 3.0.3 | MIT |
| buffer-image-size | 0.6.4 | MIT |
| cacheable | 2.5.0 | MIT |
| callsites | 3.1.0 | MIT |
| chai | 6.2.2 | MIT |
| chardet | 2.2.0 | MIT |
| chokidar | 5.0.0 | MIT |
| cli-cursor | 5.0.0 | MIT |
| cli-truncate | 5.2.0 | MIT |
| color-convert | 2.0.1 | MIT |
| color-name | 1.1.4 | MIT |
| colord | 2.9.3 | MIT |
| colorette | 2.0.20 | MIT |
| commander | 10.0.1, 14.0.3 | MIT |
| compare-versions | 6.1.1 | MIT |
| confbox | 0.1.8, 0.2.4 | MIT |
| config-chain | 1.1.13 | MIT |
| convert-source-map | 2.0.0 | MIT |
| cosmiconfig | 9.0.2 | MIT |
| cross-spawn | 7.0.6 | MIT |
| css-functions-list | 3.3.3 | MIT |
| css-tree | 3.2.1 | MIT |
| cssesc | 3.0.0 | MIT |
| csstype | 3.2.3 | MIT |
| dayjs | 1.11.21 | MIT |
| debug | 4.4.3 | MIT |
| deep-is | 0.1.4 | MIT |
| detect-indent | 6.1.0 | MIT |
| detect-libc | 2.1.2 | Apache-2.0 |
| dir-glob | 3.0.1 | MIT |
| dom-serializer | 2.0.0 | MIT |
| domelementtype | 2.3.0 | BSD-2-Clause |
| domhandler | 5.0.2 | BSD-2-Clause |
| domutils | 3.0.1 | BSD-2-Clause |
| eastasianwidth | 0.2.0 | MIT |
| echarts | 6.1.0 | Apache-2.0 |
| editorconfig | 1.0.7 | MIT |
| element-plus | 2.14.2 | MIT |
| emoji-regex | 10.6.0, 8.0.0, 9.2.2 | MIT |
| enhanced-resolve | 5.21.6 | MIT |
| enquirer | 2.4.1 | MIT |
| entities | 4.3.0, 7.0.1 | BSD-2-Clause |
| env-paths | 2.2.1 | MIT |
| environment | 1.1.0 | MIT |
| error-ex | 1.3.4 | MIT |
| es-errors | 1.3.0 | MIT |
| es-module-lexer | 2.3.0 | MIT |
| escape-string-regexp | 4.0.0 | MIT |
| eslint | 10.6.0 | MIT |
| eslint-config-prettier | 10.1.8 | MIT |
| eslint-plugin-tsdoc | 0.5.2 | MIT |
| eslint-plugin-vue | 10.9.2 | MIT |
| eslint-scope | 9.1.2 | BSD-2-Clause |
| eslint-visitor-keys | 3.4.3, 5.0.1 | Apache-2.0 |
| espree | 11.2.0 | BSD-2-Clause |
| esprima | 4.0.1 | BSD-2-Clause |
| esquery | 1.7.0 | BSD-3-Clause |
| esrecurse | 4.3.0 | BSD-2-Clause |
| estraverse | 5.3.0 | BSD-2-Clause |
| estree-walker | 2.0.2, 3.0.3 | MIT |
| esutils | 2.0.3 | BSD-2-Clause |
| eventemitter3 | 5.0.4 | MIT |
| expect-type | 1.4.0 | Apache-2.0 |
| exsolve | 1.1.0 | MIT |
| extendable-error | 0.1.7 | MIT |
| fast-deep-equal | 3.1.3 | MIT |
| fast-glob | 3.3.3 | MIT |
| fast-json-stable-stringify | 2.1.0 | MIT |
| fast-levenshtein | 2.0.6 | MIT |
| fast-uri | 3.1.3 | BSD-3-Clause |
| fastest-levenshtein | 1.0.16 | MIT |
| fastq | 1.20.1 | ISC |
| fdir | 6.5.0 | MIT |
| file-entry-cache | 11.1.5, 8.0.0 | MIT |
| fill-range | 7.1.1 | MIT |
| find-up | 4.1.0, 5.0.0 | MIT |
| flat-cache | 4.0.0, 6.1.23 | MIT |
| flatted | 3.4.2 | ISC |
| foreground-child | 3.1.0 | ISC |
| fs-extra | 7.0.1, 8.1.0 | MIT |
| fsevents | 2.3.2, 2.3.3 | MIT |
| function-bind | 1.1.2 | MIT |
| get-east-asian-width | 1.6.0 | MIT |
| glob | 10.5.0 | ISC |
| glob-parent | 5.1.2, 6.0.2 | ISC |
| global-modules | 2.0.0 | MIT |
| global-prefix | 3.0.0 | MIT |
| globals | 17.7.0 | MIT |
| globby | 11.0.0, 16.2.1 | MIT |
| globjoin | 0.1.4 | MIT |
| graceful-fs | 4.2.11 | ISC |
| happy-dom | 20.10.6 | MIT |
| has-flag | 5.0.1 | MIT |
| hashery | 1.5.1 | MIT |
| hasown | 2.0.4 | MIT |
| hookified | 1.15.1, 2.2.0 | MIT |
| hosted-git-info | 8.1.0 | ISC |
| html-tags | 5.1.0 | MIT |
| htmlparser2 | 8.0.0 | MIT |
| human-id | 4.2.0 | MIT |
| husky | 9.1.7 | MIT |
| iconv-lite | 0.7.3 | MIT |
| ignore | 5.1.4, 5.2.0, 7.0.5 | MIT |
| immutable | 5.1.9 | MIT |
| import-fresh | 3.3.0 | MIT |
| import-meta-resolve | 4.2.0 | MIT |
| imurmurhash | 0.1.4 | MIT |
| ini | 1.3.8 | ISC |
| is-arrayish | 0.2.1 | MIT |
| is-core-module | 2.16.2 | MIT |
| is-extglob | 2.1.1 | MIT |
| is-fullwidth-code-point | 3.0.0, 5.1.0 | MIT |
| is-glob | 4.0.3 | MIT |
| is-number | 7.0.0 | MIT |
| is-path-inside | 4.0.0 | MIT |
| is-plain-object | 5.0.0 | MIT |
| is-subdir | 1.1.1 | MIT |
| is-windows | 1.0.2 | MIT |
| isexe | 2.0.0 | ISC |
| jackspeak | 3.1.2 | BlueOak-1.0.0 |
| jiti | 2.7.0 | MIT |
| jju | 1.4.0 | MIT |
| js-beautify | 1.15.4 | MIT |
| js-cookie | 3.0.8 | MIT |
| js-tokens | 4.0.0, 9.0.0 | MIT |
| js-yaml | 3.15.0, 4.3.0 | MIT |
| json-buffer | 3.0.1 | MIT |
| json-parse-even-better-errors | 2.3.0 | MIT |
| json-schema-traverse | 0.4.1, 1.0.0 | MIT |
| json-stable-stringify-without-jsonify | 1.0.1 | MIT |
| jsonfile | 4.0.0 | MIT |
| keyv | 4.5.4, 5.6.0 | MIT |
| kind-of | 6.0.3 | MIT |
| known-css-properties | 0.37.0 | MIT |
| kolorist | 1.8.0 | MIT |
| levn | 0.4.1 | MIT |
| lightningcss | 1.32.0 | MPL-2.0 |
| lightningcss-android-arm64 | 1.32.0 | MPL-2.0 |
| lightningcss-darwin-arm64 | 1.32.0 | MPL-2.0 |
| lightningcss-darwin-x64 | 1.32.0 | MPL-2.0 |
| lightningcss-freebsd-x64 | 1.32.0 | MPL-2.0 |
| lightningcss-linux-arm-gnueabihf | 1.32.0 | MPL-2.0 |
| lightningcss-linux-arm64-gnu | 1.32.0 | MPL-2.0 |
| lightningcss-linux-arm64-musl | 1.32.0 | MPL-2.0 |
| lightningcss-linux-x64-gnu | 1.32.0 | MPL-2.0 |
| lightningcss-linux-x64-musl | 1.32.0 | MPL-2.0 |
| lightningcss-win32-arm64-msvc | 1.32.0 | MPL-2.0 |
| lightningcss-win32-x64-msvc | 1.32.0 | MPL-2.0 |
| lines-and-columns | 1.2.4 | MIT |
| lint-staged | 16.4.0 | MIT |
| listr2 | 9.0.5 | MIT |
| local-pkg | 1.2.1 | MIT |
| locate-path | 5.0.0, 6.0.0 | MIT |
| lodash | 4.18.1 | MIT |
| lodash-es | 4.18.1 | MIT |
| lodash-unified | 1.0.3 | MIT |
| lodash.startcase | 4.4.0 | MIT |
| lodash.truncate | 4.4.2 | MIT |
| log-update | 6.1.0 | MIT |
| lru-cache | 10.2.0 | ISC |
| magic-string | 0.30.21 | MIT |
| mathml-tag-names | 4.0.0 | MIT |
| mdn-data | 2.27.1 | CC0-1.0 |
| memoize-one | 6.0.0 | MIT |
| meow | 14.1.0 | MIT |
| merge2 | 1.4.1 | MIT |
| micromatch | 4.0.8 | MIT |
| mimic-function | 5.0.1 | MIT |
| minimatch | 10.2.5 | BlueOak-1.0.0 |
| minimatch | 9.0.9 | ISC |
| minipass | 7.1.3 | BlueOak-1.0.0 |
| mlly | 1.8.2 | MIT |
| mri | 1.2.0 | MIT |
| ms | 2.1.3 | MIT |
| muggle-string | 0.4.1 | MIT |
| nanoid | 3.3.15 | MIT |
| natural-compare | 1.4.0 | MIT |
| node-addon-api | 7.0.0 | MIT |
| nopt | 7.2.1 | ISC |
| normalize-path | 3.0.0 | MIT |
| normalize-wheel-es | 1.2.0 | BSD-3-Clause |
| npm-package-arg | 12.0.2 | ISC |
| nth-check | 2.1.1 | BSD-2-Clause |
| obug | 2.1.3 | MIT |
| onetime | 7.0.0 | MIT |
| optionator | 0.9.4 | MIT |
| outdent | 0.5.0 | MIT |
| p-filter | 2.1.0 | MIT |
| p-limit | 2.2.0, 3.0.2 | MIT |
| p-locate | 4.1.0, 5.0.0 | MIT |
| p-map | 2.0.0 | MIT |
| p-try | 2.2.0 | MIT |
| package-json-from-dist | 1.0.1 | BlueOak-1.0.0 |
| package-manager-detector | 0.2.0 | MIT |
| parent-module | 1.0.1 | MIT |
| parse-json | 5.2.0 | MIT |
| path-browserify | 1.0.1 | MIT |
| path-exists | 4.0.0 | MIT |
| path-key | 3.1.1 | MIT |
| path-parse | 1.0.7 | MIT |
| path-scurry | 1.11.1 | BlueOak-1.0.0 |
| path-type | 4.0.0 | MIT |
| pathe | 2.0.3 | MIT |
| picocolors | 1.1.1 | ISC |
| picomatch | 2.3.2, 4.0.5 | MIT |
| pify | 4.0.1 | MIT |
| pkg-types | 1.3.1, 2.3.1 | MIT |
| playwright | 1.61.1 | Apache-2.0 |
| playwright-core | 1.61.1 | Apache-2.0 |
| postcss | 8.5.16 | MIT |
| postcss-html | 1.8.1 | MIT |
| postcss-media-query-parser | 0.2.3 | MIT |
| postcss-resolve-nested-selector | 0.1.6 | MIT |
| postcss-safe-parser | 6.0.0, 7.0.1 | MIT |
| postcss-scss | 4.0.9 | MIT |
| postcss-selector-parser | 7.1.4 | MIT |
| postcss-value-parser | 4.2.0 | MIT |
| prelude-ls | 1.2.1 | MIT |
| prettier | 2.7.1, 3.9.5 | MIT |
| proc-log | 5.0.0 | ISC |
| proto-list | 1.2.4 | ISC |
| punycode | 2.3.1 | MIT |
| qified | 0.10.1 | MIT |
| quansync | 0.2.11 | MIT |
| queue-microtask | 1.2.3 | MIT |
| read-yaml-file | 1.1.0 | MIT |
| readdirp | 5.0.0 | MIT |
| require-from-string | 2.0.2 | MIT |
| resolve | 1.22.12 | MIT |
| resolve-from | 4.0.0, 5.0.0 | MIT |
| restore-cursor | 5.1.0 | MIT |
| reusify | 1.1.0 | MIT |
| rfdc | 1.4.1 | MIT |
| rimraf | 5.0.5 | ISC |
| rolldown | 1.1.5 | MIT |
| run-parallel | 1.2.0 | MIT |
| safer-buffer | 2.1.2 | MIT |
| sass | 1.101.0 | MIT |
| semver | 7.8.5 | ISC |
| shebang-command | 2.0.0 | MIT |
| shebang-regex | 3.0.0 | MIT |
| siginfo | 2.0.0 | ISC |
| signal-exit | 4.1.0 | ISC |
| slash | 3.0.0, 5.1.0 | MIT |
| slice-ansi | 4.0.0, 7.1.2, 8.0.0 | MIT |
| source-map-js | 1.2.1 | BSD-3-Clause |
| spawndamnit | 3.0.1 | MIT* |
| sprintf-js | 1.0.3 | BSD-3-Clause |
| stackback | 0.0.2 | MIT |
| std-env | 4.2.0 | MIT |
| string-argv | 0.3.2 | MIT |
| string-width | 4.2.3, 5.1.2, 7.2.0, 8.2.2 | MIT |
| strip-ansi | 6.0.1, 7.2.0 | MIT |
| strip-bom | 3.0.0 | MIT |
| stylelint | 17.14.0 | MIT |
| stylelint-config-recommended | 18.0.0 | MIT |
| stylelint-config-recommended-scss | 17.0.1 | MIT |
| stylelint-config-standard | 40.0.0 | MIT |
| stylelint-config-standard-scss | 17.0.0 | MIT |
| stylelint-scss | 7.2.0 | MIT |
| supports-color | 10.2.2 | MIT |
| supports-hyperlinks | 4.5.0 | MIT |
| supports-preserve-symlinks-flag | 1.0.0 | MIT |
| svg-tags | 1.0.0 | MIT |
| table | 6.9.0 | BSD-3-Clause |
| tailwindcss | 4.3.2 | MIT |
| tapable | 2.3.3 | MIT |
| term-size | 2.2.1 | MIT |
| tinybench | 2.9.0 | MIT |
| tinyexec | 1.2.4 | MIT |
| tinyglobby | 0.2.17 | MIT |
| tinyrainbow | 3.1.0 | MIT |
| to-regex-range | 5.0.1 | MIT |
| ts-api-utils | 2.5.0 | MIT |
| tslib | 2.3.0, 2.8.1 | 0BSD |
| type-check | 0.4.0 | MIT |
| typescript | 6.0.3 | Apache-2.0 |
| typescript-eslint | 8.63.0 | MIT |
| ufo | 1.6.4 | MIT |
| undici-types | 6.21.0 | MIT |
| unicorn-magic | 0.4.0 | MIT |
| universalify | 0.1.2 | MIT |
| unplugin | 2.3.2 | MIT |
| unplugin-dts | 1.0.3 | MIT |
| uri-js | 4.4.1 | BSD-2-Clause |
| util-deprecate | 1.0.2 | MIT |
| validate-npm-package-name | 6.0.2 | ISC |
| vite | 8.1.4 | MIT |
| vite-plugin-dts | 5.0.3 | MIT |
| vitest | 4.1.10 | MIT |
| vscode-uri | 3.1.0 | MIT |
| vue | 3.5.39 | MIT |
| vue-component-type-helpers | 3.3.7 | MIT |
| vue-eslint-parser | 10.4.1 | MIT |
| vue-tsc | 3.3.7 | MIT |
| webpack-virtual-modules | 0.6.2 | MIT |
| whatwg-mimetype | 3.0.0 | MIT |
| which | 1.3.1, 2.0.1 | ISC |
| why-is-node-running | 2.3.0 | MIT |
| word-wrap | 1.2.5 | MIT |
| wrap-ansi | 7.0.0, 8.1.0, 9.0.2 | MIT |
| write-file-atomic | 7.0.1 | ISC |
| ws | 8.21.0 | MIT |
| xml-name-validator | 4.0.0 | Apache-2.0 |
| yaml | 2.9.0 | ISC |
| zrender | 6.1.0 | BSD-3-Clause |

\* `spawndamnit@3.0.1` 的 package.json 使用 `SEE LICENSE IN LICENSE`，其随包 LICENSE 文本已人工复核为 MIT License。

